...
**`xray-dog-fetcher` is the exception**: it uses a self-owned copy of this same layer instead of the shared ARN above, to demonstrate how to avoid a live cross-account layer reference at deploy time (some environments block attaching layers published by a foreign account, and prod's own build/deploy can't make the AWS API call to fetch one on the fly either). `lambda/xray-dog-fetcher/otel-layer.zip` is a **vendored, checked-in artifact** — AWS's real, official layer content, not a rebuild from source. It's produced by a three-step handoff:

0. **`find-latest-otel-layer-version.sh <ver-x-y-z> [region]`** — run by hand first, whenever bumping to a new ADOT release line (e.g. `ver-1-30-2`), to find the latest published version *number* for that line, for both `amd64` and `arm64`. This is needed because the layer is published by a foreign account (AWS's own, `901920570463`): `aws lambda list-layer-versions` only lists versions in the *caller's* account, so there's no direct "list versions of someone else's layer" API. The only cross-account read the layer's public resource policy allows is `get-layer-version` for one specific version number at a time. The script probes version numbers upward (`aws lambda get-layer-version --version-number N`) and reports the highest one that resolves; it tolerates gaps (a few consecutive failures before giving up), because AWS's own publishes aren't always contiguous — e.g. `aws-otel-nodejs-arm64-ver-1-30-0` is missing versions 3 and 5 but has versions up to 8. A `GetLayerVersion` failure and a real permissions problem look identical (both come back `AccessDeniedException` — there's no separate "not found" response for a resource-policy check), but since lower version numbers succeed with the same credentials, a failure on a higher number reliably means "doesn't exist yet". Feed its output into the next script's `LAYER_NAME`/`LAYER_VERSION` variables.
1. **`resolve-otel-layer-url.sh`** — run by hand, somewhere with AWS CLI + credentials (not prod's own pipeline). It calls `aws lambda get-layer-version` against the public ARN (a metadata read, not an attach) to get a presigned S3 URL for the layer's zip content, and writes it to `otel-layer-download-manifest.txt` along with an explicit expiry timestamp — the URL is only valid for about 10 minutes. If the handoff below doesn't happen in time, just re-run this script for a fresh URL.
2. **A separate download team** fetches that URL (a plain `curl`, no AWS credentials needed once it's presigned) before it expires, saves the result as `lambda/xray-dog-fetcher/otel-layer.zip`, and adds it to this repo. No build step — it's AWS's ready-to-use layer content as-is.

`frontend_stack.py`'s `AdotLayerDogFetcher` builds a normal `LayerVersion` from that zip via `Code.from_asset` (CDK uploads a `.zip` path as-is, no local unzip/rezip) instead of `from_layer_version_arn`.

None of this runs as part of `deploy.sh`/prod's own pipeline — it's refreshed manually, out-of-band, whenever the vendored layer needs updating, then the result is committed — the same way you'd bump any other vendored dependency. The runtime behavior is identical to the ARN-based layer — it's still a Layer mounted at `/opt` with the same internal layout, since a Lambda Extension (the embedded collector binary) can only be discovered by Lambda from `/opt/extensions/`, which only a Layer can populate. `xray-invoker` and `xray-s3-writer` keep using the AWS-managed ARN for contrast.
