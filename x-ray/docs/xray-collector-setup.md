...
`lambda/xray-dog-fetcher/otel-layer.zip` is a **vendored, checked-in artifact built entirely from source** — no AWS credentials, AWS API calls, or even `git` anywhere in producing it, only plain HTTPS zip downloads (some environments allow that but not `git clone`). `lambda/xray-dog-fetcher/build-otel-layer.sh` downloads the public [`aws-observability/aws-otel-lambda`](https://github.com/aws-observability/aws-otel-lambda) repo as a zip via GitHub's codeload archive endpoint (itself a downstream patch of [`open-telemetry/opentelemetry-lambda`](https://github.com/open-telemetry/opentelemetry-lambda) that swaps in AWS's own X-Ray-specific collector components — the `awsxrayexporter` and `sigv4authextension`, confirmed by inspecting the built binary). Since a repo's zip archive doesn't include its git submodules' content, the script separately resolves which exact commit the `opentelemetry-lambda` submodule is pinned to via GitHub's public Contents API (a plain unauthenticated GET, not a git operation) and downloads that commit as its own zip too. It then compiles the Go-based collector extension and the Node.js SDK/wrapper locally and merges them into one layer zip — the same process AWS's own CI uses to produce the ARN we'd otherwise reference. `frontend_stack.py`'s `AdotLayerDogFetcher` builds a normal `LayerVersion` from that zip via `Code.from_asset` (CDK uploads a `.zip` path as-is, no local unzip/rezip) instead of `from_layer_version_arn`.

The build doesn't run as part of `deploy.sh`/prod's own pipeline — it's refreshed manually, out-of-band, on a machine with `curl`, `jq`, `go`, `node`/`npm`, `patch`, and `zip`/`unzip` (all standard dev tooling, nothing AWS-specific, no `git` required), whenever the vendored layer needs updating, then the result is committed — the same way you'd bump any other vendored dependency. The runtime behavior is identical to the ARN-based layer — it's still a Layer mounted at `/opt` with the same internal layout, since a Lambda Extension (the embedded collector binary) can only be discovered by Lambda from `/opt/extensions/`, which only a Layer can populate. `xray-invoker` and `xray-s3-writer` keep using the AWS-managed ARN for contrast.
