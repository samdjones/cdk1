...
**`xray-dog-fetcher` is the exception**: it uses a self-built copy of this same layer instead of the shared ARN above, to demonstrate how to avoid a live cross-account layer reference at deploy time — some environments block attaching layers published by a foreign account, and can't make *any* AWS API call (not even a read-only one, and not via the aws CLI or any other tool) as part of their own build/deploy either, which rules out fetching AWS's prebuilt artifact on the fly.

`lambda/xray-dog-fetcher/otel-layer.zip` is a **vendored, checked-in artifact built entirely from source** — no AWS credentials or AWS API calls, no `git`, and (per the split below) no team downloading raw source zips is the same team that runs the actual build. The process is three phases, each a separate script/handoff:

1. **`resolve-otel-layer-urls.sh`** — the only phase that talks to the network, and only to small metadata endpoints (GitHub's commits/contents APIs, plain unauthenticated GETs — no bulk downloads). It resolves the exact commit SHAs for the public [`aws-observability/aws-otel-lambda`](https://github.com/aws-observability/aws-otel-lambda) repo (itself a downstream patch of [`open-telemetry/opentelemetry-lambda`](https://github.com/open-telemetry/opentelemetry-lambda) that swaps in AWS's own X-Ray-specific collector components — the `awsxrayexporter` and `sigv4authextension`, confirmed by inspecting the built binary) and its `opentelemetry-lambda` submodule (a repo's zip archive doesn't include its git submodules' content, so the submodule's pinned commit has to be resolved separately via GitHub's Contents API), then writes `otel-layer-download-manifest.txt` — two rows of `<filename> <url>`, each URL a GitHub codeload zip-archive download (`.../archive/<sha>.zip`, no `git clone` involved).
2. **Another team downloads** each URL in that manifest and adds the resulting files to this repo under `lambda/xray-dog-fetcher/otel-build-inputs/<filename>`, matching the manifest's exact names.
3. **`build-otel-layer.sh`** — runs entirely offline once those two zips are staged. It extracts them, applies AWS's patches, compiles the Go-based collector extension and the Node.js SDK/wrapper locally, and merges them into `otel-layer.zip` — the same process AWS's own CI uses to produce the ARN we'd otherwise reference.
